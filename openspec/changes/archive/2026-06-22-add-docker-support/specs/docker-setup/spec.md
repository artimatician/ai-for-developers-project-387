...
#### Scenario: CI build job produces a valid image
- **WHEN** running `docker build --target prod -t calendar-ci . --build-arg PORT=8080`
- **THEN** the build SHALL succeed without errors

#### Scenario: CI job runs container and verifies health
- **WHEN** running the built image as a container with `-e PRODUCTION_DB=true -p 8080:8080`
- **THEN** `GET http://localhost:8080/api/health` SHALL return a 200 response within 30 seconds

#### Scenario: CI job verifies frontend is served
- **WHEN** running the container with default settings
- **THEN** `GET http://localhost:8080/` SHALL return a 200 response with `<!DOCTYPE html`

#### Scenario: CI job cleans up container on success or failure
- **WHEN** the CI job finishes (whether passing or failing)
- **THEN** the container SHALL be stopped and removed
