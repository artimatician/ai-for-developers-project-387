# owner-event-types Specification

## Purpose

Owner event type management page with CRUD operations, soft-delete toggle, and consistent visual styling.
## Requirements
...
- AND the page SHALL display a table with columns for Name, Duration, Timezone, Active status, and Actions
...
- AND each row SHALL show the event type name, duration (e.g. "30 min"), timezone, and a toggle switch for Active status
...
- THEN a modal SHALL open with fields for Name (required), Description (optional), Timezone (optional, defaults to "UTC"), and Duration (optional, defaults to 30, step 15, range 15–480)
...
- THEN a modal SHALL open with all current values pre-filled (name, description, timezone, duration)
- WHEN the owner modifies the duration and submits
- THEN the system SHALL update the event type via PATCH `/api/owner/event-types/{id}`
- AND the table SHALL reflect the updated duration
- AND a success notification SHALL be displayed
